...
import {
    PauseHelper
} from '../../../src/utils/pause-helper';
import {
    Services
} from '../../../src/services';
import {
    base64EncodeFromBytes
} from '../../../src/core/types/string/base64.js';
import {
    createCustomEvent,
    getData,
    listen
...
import {
    utf8Encode
} from '../../../src/core/types/string/bytes.js';
...
export class AmpIframe extends AMP.BaseElement {
    ...
    assertSource_(src, containerSrc, sandbox = '') {
        ...
        userAssert(!this.sandboxContainsToken_(sandbox, 'allow-same-origin') ||
    ...
    transformSrc_(src) {
        ...
        const {
            protocol,
            hash
        } = Services.urlForDoc(this.element).parse(src);
    ...
    transformSrcDoc_(srcdoc, sandbox) {
        ...
        userAssert(!(' ' + sandbox + ' ').match(/\s+allow-same-origin\s+/i),
    ...
    /** @override */
    layoutCallback() {
        userAssert(!this.isDisallowedAsAd_,
        ...
        if (this.isTrackingFrame_) {
            if (!this.getAmpDoc().registerSingleton(
