...
import {
    FiniteStateMachine
} from '../core/data-structures/finite-state-machine';
import {
    FocusHistory
} from '../focus-history';
import {
...
import {
    ieIntrinsicCheckAndFix
} from './ie-intrinsic-bug';
import {
    ieMediaCheckAndFix
} from './ie-media-bug';
import {
...
export class ResourcesImpl {
    ...
    mutateWork_() {
        ...
        if (this.requestsChangeSize_.length > 0) {
            ...
            for (let i = 0; i < requestsChangeSize.length; i++) {
                ...
                let {
                    top: topUnchangedBoundary,
                    bottom: bottomDisplacedBoundary
                } = box;
    ...
    discoverWork_() {
        ...
        const visibleRect = this.visible_ ? // When the doc is visible, consider the viewport to be 25% larger,
        ...
        if (this.visible_ && this.isIdle_(now)) {
            ...
            for (
                let i = 0; i < this.resources_.length && idleScheduledCount < 4; i++
            ) {
                const r = this.resources_[i];
                if (
