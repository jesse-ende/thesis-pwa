...
# BUILD DATE: Thu Nov 12 2020 17:04:46 GMT-0800 (Pacific Standard Time)
...
var enable_tracking = !0,
    isTest = -1 != location.host.indexOf("-stage") || -1 != location.host.indexOf("dev-") || -1 != location.host.indexOf("-dev") || -1 != location.host.indexOf("webstandards-us") || -1 != location.host.indexOf("localhost");
try {
    ...
    (function() {
        ...
        function getCookie(cookieKey) {
            for (var name = cookieKey + "=", cookieArray = document.cookie.split(";"), i = 0; i < cookieArray.length; i++) {
                for (var c = cookieArray[i];
        ...
        function getLocalStorageItem(storageKey) {
            return "undefined" != typeof Storage ? localStorage.getItem(storageKey) : null
        }
...
function s_getCookieData(label) {
    for (var labelLen = label.length, cLen = document.cookie.length, i = 0, cEnd; i < cLen;) {
        var j = i + labelLen;
        if (document.cookie.substring(i, j) == label) {
            cEnd = document.cookie.indexOf(";", j); - 1 == cEnd && (cEnd = document.cookie.length);
            j++;
...
if (enable_tracking) {
    ...
    function s_setAccount() {
        ...
        if (isInternalStoryhub()) {
            ...
            sa[2] = "en-us";
            s_checkdev() ? sa[0] = "oracleinternalmysitesdev" : sa[0] = "oracleinternalmysites"
        }
        siteID = sa[1];
        language = sa[2];
    ...
    function isInternalStoryhub() {
        var isDev = !1,
            domains = ["storyhub-cecdevaccount.cec.ocp.oc-test.com", "myapps.oracle.com", "myapps-stage.oracle.com"],
            al = domains.length;
        for (i = 0; i < al; i++)
    ...
    function s_checkdev() {
        var isDev = !1,
            devSites = ["-stage", "-dev", "dev-", "localhost", "127.0.0.1", "-sites", "storyhub-cecdevaccount", "adc2171266.us.oracle.com", "webstandards-us"],
            al = devSites.length;
        for (i = 0; i < al; i++)
    ...
    function s_prePlugins(s) {
        s_oraVer(":storyhub", ":1.12");
        s.pageName && (s.pageName.indexOf(":askoraclesearch") > -1 || s.pageName.indexOf(":Search") > -1) || setPageName(s);
        s.pageName.indexOf(":Search") > -1 ? s.eVar26 = siteID + ":search" : s.eVar26 = siteID;
        oraSetInternalFilters();
        var url = s.linkObject;
        if (isInternalStoryhub())
            if (!url || url.closest("[data-trackas]") || !url.href || -1 == url.href.indexOf("http") && -1 == url.href.indexOf("https")) {
                s.prop8 = "";
                s.eVar53 = "";
                s.prop22 = ""
            } else {
                if (-1 != url.href.indexOf(location.hostname)) {
                    ...
                    linkText = linkText ? linkText.toLowerCase().replace(/[*,\r\n\t ]+/gi, "-") : linkText;
                    s.linkType = "o";
                    s.prop8 = s.pageName;
    ...
    function setPageName(s) {
        ...
        void 0 != ora_product ? s.pageName = siteID + ":" + language + ":" + path.toLowerCase().replace(/ /g, "-").replace(/-+/g, "-") + ora_product : "/index.html" == window.location.pathname ? s.pageName = siteID + ":" + language + ":/" : s.pageName = siteID + ":" + language + ":" + path.toLowerCase().replace(/ /g, "-").replace(/-+/g, "-");
        s.eVar26 = siteID
    }
    ...
    function s_postPlugins(s) {
        ...
        isInternalStoryhub() || (s.eVar26 = siteID + ":" + language.split("-")[0]); - 1 == s.linkDownloadFileTypes.indexOf("deb") && (s.linkDownloadFileTypes += ",deb");
        window.dataLayerObject && window.dataLayerObject.page && (s.eVar92 = window.dataLayerObject.page.variation)
    }
    ...
    function oraSetInternalFilters() {
        ...
        (location.href.indexOf(":8888") || location.href.indexOf("webstandards-us") || location.href.indexOf("localhost")) && (s.linkInternalFilters = "javascript:,localhost,webstandards-us.oracle.com,.oracle.,.cec.ocp.oc-test.")
    }
}
document.addEventListener("click", function(event) {
    var srcEle = event.target || event.srcElement,
        lbl = "",
        targetEle = findParentByTagName(srcEle, "A") || findParentByTagName(srcEle, "BUTTON"),
        o = targetEle;
    if (targetEle && targetEle.closest("*[data-trackas]") && "notrack" != getAttribute(o, "data-lbl")) {
        ...
        d = "hnav" == d || "header" == d || "hp07" == d || "hp08" == d || "hp09" == d || "menu" == d || "hp10" == d || "cn24" == d || "hp11" == d || "cb50" == d || "cn27" == d || "cb25" == d || "ch05" == d || "cw58" == d || "cw59" == d || "cb65" == d || "cb66" == d || "cb67" == d ? ":" : "-";
        if (!o.hasAttribute("data-trackas"))
            for (; o.parentNode;) {
                o = o.parentNode;
                o.hasAttribute("data-lbl") && (lbl = o.getAttribute("data-lbl") + d + lbl);
                if (o.hasAttribute("data-trackas")) break
            }
        lbl = lbl.toLowerCase().replace(/ /g, "-").replace(/-+/g, "-"); - 1 != lbl.indexOf("mpd-") && (lbl = lbl.replace(/mpd-/g, ""));
        var sec = o.hasAttribute("data-trackas") ? o.getAttribute("data-trackas") : o.closest("*[data-trackas]").getAttribute("data-trackas"); - 1 != sec.indexOf("mpd-") && (sec = sec.replace(/mpd-/g, ""));
        "cw22-carouseltop" != sec && window.navTrack && navTrack(siteID, language, sec, lbl);
        if ("true" == getAttribute(o, "data-pgreset")) {
            s.clearVars();
            if (0 == getAttribute(o, "href").indexOf("#") && s.pageName) {
                if (document.body.hasAttribute("data-pgname")) var pn = getAttribute(o, "data-pgname");
                else {
            ...
            oraSetInternalFilters();
            s.linkInternalFilters = s.linkInternalFilters + "," + getAttribute(o, "href");
            var s_code = s.t();
...
function findParentByTagName(element, tagName) {
    for (var parent = element; null !== parent && parent.tagName !== tagName.toUpperCase();) parent = parent.parentNode;
    return parent
}
